        ...
        attach: function() {
            ...
            if (c.autoPlay) {
                ...
                function(i, elem) {
                    ...
                    // 鼠标移入不间断
                    self.nextBtn.bind("mouseover",
                    function(e) {
                        self.stop()
                    });
                    self.nextBtn.bind("mouseout",
                    function(e) {
                        self.play()
                    });

                    self.prevBtn.bind("mouseover",
                    function(e) {
                        self.stop()
                    });
                    self.prevBtn.bind("mouseout",
                    function(e) {
                        self.play()
                    });
    ...
    $.extend(Tab.effect, {
        ...
        slide3d: {
            ...
            onchange: function() {
                ...
                //console.log(Math.abs(to - from) > self.target.length / 2);
                //if(to >= self.target.length) to = 1;
                self.curPage = to;
                var pointerOffset = 0;
                merge: if (c.merge) {
                    var across = to - from,
                ...
                var resetStyle = function() {
                	...
                	if(self.curPage === self.length) {
	                	setTimeout(function(){
	                    	self.content.css({'left': 0, 'margin-left': 0});
	                    	self.curPage = 0
	                	}, 250)
                	}
                };
                window.setTimeout(function() {
                    ...
                    o[self.prop] = -pos + "px";
                    self.content.stop();
                    self.content.animate(o, self.config.animateTime, "easeInOutExpo", resetStyle)
                },
                250);
                window.setTimeout(function() {
